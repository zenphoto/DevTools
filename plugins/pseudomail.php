...
$plugin_disable = (zp_has_filter('sendmail') && !getoption('zp_plugin_pseudomail'))?sprintf(gettext('Only one Email handler plugin may be enalbed. <a href="#%1$s"><code>%1$s</code></a> is already enabled.'),stripSuffix(get_filterScript('sendmail'))):'';

if ($plugin_disable) {
	setOption('zp_plugin_pseudomail', 0);
} else {
